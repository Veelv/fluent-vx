...
- **Code Generation**: Fixed duplicate vxData variable declaration in generated JavaScript code
- **Inline Scripts**: Separated inline JavaScript generation to avoid variable conflicts in dev mode
- **Parser Robustness**: Enhanced lexer whitespace handling for block declarations (#style, #data, etc.)

### 🔧 Changed

- **Code Generator**: Refactored to prevent duplicate declarations between inline and separate JS generation
- **Build Process**: Improved separation between development inline scripts and production builds
...
- **HMR Architecture**: Refactored HMR to use same port as dev server instead of separate WebSocket server
- **Parser Improvements**: Fixed lexer to properly handle whitespace after # symbols in block declarations
- **WebSocket Integration**: Integrated WebSocket server directly into Express server for better reliability

### 🔧 Changed

- **Server Architecture**: Dev server now handles both HTTP and WebSocket connections on the same port
- **File Watching**: Removed external chokidar dependency, using custom Node.js fs.watch implementation
- **Parser**: Improved tokenization to handle #style, #data, etc. blocks more robustly
...
- **Dev Server Dependencies**: Removed chokidar dependency entirely to eliminate external library dependencies
- **Custom File Watcher**: Implemented professional file watching using Node.js built-in fs.watch with debouncing and proper event handling
- **TypeScript Types**: Updated all type definitions to work without external dependencies
...
- **Static Site Generation**: Build now generates HTML files for all routes
- **Route-based Output**: Each page gets its own static file (index.html, about.html, etc.)
- **Zero-config Deployment**: Generated files ready for any static hosting

### 🔧 Changed

- **Build Process**: Now builds all discovered routes instead of single app file
- **Output Structure**: Multiple HTML files in .vx/ directory
